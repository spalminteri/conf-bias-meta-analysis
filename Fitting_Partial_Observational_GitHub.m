...
for i = 1:length(a)
    ...
    if (a(i))~=1.5 % to exclude missed reponses
        ...
        if model==1
            PEc =  r(i) - Q(s(i),a(i));
            if obs(i)==1 % if the trial is not observational there is calculation of the likelihood of observing the action 
                
                lik = lik + log (1/(1+ exp(-beta*(Q(s(i),a(i))-Q(s(i),3-a(i))))));
                
                % and the asymmetric learnign update is used 
                
                Q(s(i),a(i)) = Q(s(i),a(i)) + lr1 * PEc * (PEc>0) +  lr2 * PEc * (PEc<0);
            else
                % if the trial is observational only the update with
                % dedicated learning rate 
